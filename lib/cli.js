...
/**
 * Expose command line interface as constructor and allow custom arguments.
 *
 * @alias module:Cli
 * @constructor
 * @param {String} name
...
Cli.prototype.initialize = function initialize() {
  this.cli = new Commander(this.name);
  this.cli
    ...
    .option('-p, --plugins <plugins>', 'specify the plugins to use', this.list, []);

  //
  // Allow user to scaffold default Cataloger options file.
  //
  this.cli
    .command('scaffold [path]')
    .description('scaffold a cataloger.opts file')
    .action(this.scaffold);
};
 ...
 * @api public
 */
Cli.prototype.scaffold = function scaffold(destination) {
  var target = new Scaffold({ output: destination });

  target.save(function saved() {
    [
      '',
      'Created a new `cataloger.opts` file at ' + destination,
      '',
...
cli
  ...
  .action(function scaffold(destination) {
    ...
    if (destination) require('mkdirp').sync(destination, 775);
...
/**
 * Setup additional details for watching, the actual watching is not done
 * until `parsed` is emitted.
 *
 * @param {Number} port
 * @param {Object} square current square instance
 * @api public
 */
cli.on('--watch', function watch(port, square) {
  ...
  // Everyting is parsed, start listening for file changes.
  cli.once('parsed', function parsed() {
    square.on('processing', spinner.stop);
    square.on('idle', spinner);

    var watcher = new square.Watcher(port);
  });
});
...
async.reduce(cli.args, {}, function (memo, bundle, callback) {
  ...
  ['watch'].forEach(function emit(key) {
    if (cli[key]) cli.emit('--'+ key, cli[key], square);
  });

  // Mitigate CLI configured settings to square's configuration.
  if (cli.storage) square.package.configuration.storage = cli.storage;
  if (cli.platform) square.package.configuration.platform = cli.platform;
  if (cli.distribute) square.package.configuration.distribute = cli.distribute;

  // Initialize the storage containers.
  (cli.storage || square.config.storage).forEach(function load(storage) {
  ...
  async.forEach(
      cli.platform || square.config.platform
  ...
  // Make sure we kill square completely.
  process.exit(0);
});
