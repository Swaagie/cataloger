...
Cli.prototype.initialize = function initialize() {
  ...
  cli.on('--list', this.parsers.bind(this));
  ...
  //
  if (!cli.args.length && !fs.existsSync(file)) {
    this.help();
  }
...
Cli.prototype.parsers = function parsers() {
  this.output(this.cataloger.list());
};
